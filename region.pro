...
UBUNTU_TRANSLATION_DOMAIN="region"
...
aptest.commands = QML2_IMPORT_PATH=$$OUT_PWD/backend bash $$PWD/region/tests/autopilot/run
aptest.depends  = sub-region sub-backend-App

unittest.target   = check
unittest.commands = /usr/bin/qmltestrunner -input $$PWD/backend/tests/unit -import $$OUT_PWD/backend
unittest.commands += && /usr/bin/qmltestrunner -input $$PWD/region/tests/unit -import $$OUT_PWD/backend
unittest.depends  = sub-region sub-backend-App
